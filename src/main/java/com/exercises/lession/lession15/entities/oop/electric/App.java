package com.exercises.lession.lession15.entities.oop.electric;

import java.util.Random;

public class App {
	/*
	 * Các loại hình đăng ký điện gồm: Điện sinh hoạt giá bán lẻ Điện sinh hoạt giá
	 * bán sĩ Điện kinh doanh cao áp Điện kinh doanh trung áp Điện kinh doanh thấp
	 * áp
	 */
	...
	public static void main(String[] args) {
		...
		for (int c = 0; c < n; c++) {
			if (c % 5 == 0) {// giả sử trường hợp test customerType: 0,1,2,3,4 lần lượt là khách lẻ, sỉ,
								// doanh nghiệp cao, trung và thấp áp
				RetailCustomer retailCustomer = new RetailCustomer(customer[c][0], customer[c][1], customer[c][2],
						electricCustomer[c][0], electricCustomer[c][1]);
				System.out.println(retailCustomer.toString());
				System.out.println("Số tiền phải trả là: " + retailCustomer.calElectricBill() + " VNĐ");
				System.out.println("=========================================");
			} else if (c % 5 == 1) {
				WholesaleCustomer wholesaleCustomer = new WholesaleCustomer(customer[c][0], customer[c][1],
						customer[c][2], electricCustomer[c][0], electricCustomer[c][1]);
				...
				BusinessCustomerHighPressure businessCustomerHighPressure = new BusinessCustomerHighPressure(
						customer[c][0], customer[c][1], customer[c][2], electricBusiness[c][0], electricBusiness[c][1],
						electricBusiness[c][2]);
				...
				BusinessCustomerMediumPressure businessCustomerMediumPressure = new BusinessCustomerMediumPressure(
						customer[c][0], customer[c][1], customer[c][2], electricBusiness[c][0], electricBusiness[c][1],
						electricBusiness[c][2]);
				...
				BusinessCustomerLowPressure businessCustomerLowPressure = new BusinessCustomerLowPressure(
						customer[c][0], customer[c][1], customer[c][2], electricBusiness[c][0], electricBusiness[c][1],
						electricBusiness[c][2]);
